...
async def webhook_handler(request: Request):
    ...
    # -------------------------
    # RESET IF COMPLETED AND USER SAYS HI/HELLO
    # -------------------------
    if room_id in ROOM_STATE:
        if ROOM_STATE[room_id]["stage"] == STAGE_COMPLETED:
            if text.lower() in ["hi", "hello", "hey"]:
                del ROOM_STATE[room_id]  # reset room
            else:
                return {"status": "interview_already_completed"}
    ...
    if state["stage"] == STAGE_INTRO:
        state["stage"] = STAGE_HR
        state["hr_index"] = 0
        next_question = HR_QUESTIONS[state["hr_index"]]
        state["last_question"] = next_question
        send_message(room_id, next_question)
        return {"status": "hr_started"}
    ...
    if state["stage"] == STAGE_HR:
        ...
        # HR complete → switch to TECH
        state["stage"] = STAGE_TECH
        state["tech_count"] = 0
        state["technical_qna"] = []

        # Adaptive difficulty based on intro length
        if len(text) > 120:
            state["difficulty"] = "hard"
        elif len(text) < 40:
            state["difficulty"] = "easy"
        ...
        state["technical_qna"].append({"question": next_question, "answer": ""})
        state["tech_count"] = 1
        state["last_question"] = next_question
        send_message(room_id, next_question)
        return {"status": "tech_started"}
